-- ==================================================================================
-- MIGRATION: Dungeon hospital protection, live power, farm penalty, free heal
-- ==================================================================================
-- 1. dng_001–003: zero hospital risk on failure
-- 2. Always recalculate live power (fix UI/server success rate drift)
-- 3. Level + high-success farm penalty on gold/xp/loot
-- 4. hospital_lifetime_count + free discharge for first 2 hospital visits
-- ==================================================================================

ALTER TABLE public.users
  ADD COLUMN IF NOT EXISTS hospital_lifetime_count INTEGER NOT NULL DEFAULT 0;
...
BEGIN
  ...
  -- Live power every run (keeps UI/server success rate aligned)
  ...
  -- Hospital risk preview (on failure)
  ...
  IF v_success THEN
    ...
    -- Farm penalty: overleveled players earn less in low dungeons
    ...
    v_level_mult := GREATEST(0.05, 1.0 - (v_level_gap * 0.12));
    v_success_mult := 1.0;
    IF v_success_rate >= 0.80 THEN
      v_success_mult := GREATEST(0.10, 1.0 - ((v_success_rate - 0.80) / 0.15) * 0.90);
    END IF;
    v_reward_mult := v_level_mult * v_success_mult;
...
-- Free discharge for first 2 lifetime hospital visits
...
BEGIN
    ...
    SELECT hospital_until, gems, COALESCE(hospital_lifetime_count, 0)
    INTO v_hospital_until, v_gems, v_hospital_count
    FROM public.users
    ...
    -- First 2 lifetime hospital visits discharge for free
