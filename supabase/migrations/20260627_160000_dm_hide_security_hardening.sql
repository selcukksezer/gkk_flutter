-- Harden hide_dm_message: search_path, null/system guards, revoke direct table writes

REVOKE ALL ON TABLE public.chat_dm_message_hides FROM PUBLIC;
REVOKE ALL ON TABLE public.chat_dm_message_hides FROM anon;
REVOKE ALL ON TABLE public.chat_dm_message_hides FROM authenticated;
GRANT SELECT ON TABLE public.chat_dm_message_hides TO authenticated;
...
BEGIN
  IF p_message_id IS NULL THEN
    RETURN jsonb_build_object('success', false, 'error', 'Gecersiz mesaj');
  END IF;

  v_user := public.get_current_chat_user();

  SELECT id, channel, sender_user_id, recipient_user_id, deleted_at, read_at, is_system
  INTO v_message
  ...
  IF v_message.is_system THEN
    RETURN jsonb_build_object('success', false, 'error', 'Sistem mesaji silinemez');
  END IF;
