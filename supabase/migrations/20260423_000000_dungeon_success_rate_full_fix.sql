-- =========================================================================================
-- MIGRATION: Dungeon Success Rate Full Fix
-- =========================================================================================
-- Sorunlar:
-- 1. enter_dungeon RPC'de reputation bonusu (max +2.5%) hiç uygulanmıyordu.
-- 2. users.power = 0 olduğunda fallback sadece level*500 kullanıyordu;
--    calculate_user_total_power varsa onu kullanmak daha doğru.
-- 3. Zindan 1 garantisi: power_requirement = 0 durumu TWICE kontrol ediliyordu;
--    sadece tek yerde clamp yapmak yeterli (final_rate clamp'ında).
--
-- Düzeltmeler:
--  - users.power = 0 ise calculate_user_total_power(p_player_id) çağrılır.
--  - Luck bonusundan sonra reputation bonusu eklendi: reputation * 0.0005.
--  - power_requirement = 0 için tek noktada 1.0 dönüşü (duplikat kaldırıldı).
--  - clamp(0.05, 0.95) tüm zindanlar için; power_req=0 öncesinde 1.0 döner.
-- =========================================================================================
