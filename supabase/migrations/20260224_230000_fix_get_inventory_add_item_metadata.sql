-- ============================================================
-- Fix: get_inventory should include item metadata (name, icon, stats)
...
BEGIN
    ...
    -- Include item metadata by joining to public.items
    -- Split into multiple jsonb_build_object calls to avoid exceeding 100 argument limit
    SELECT jsonb_agg(
        jsonb_build_object(
            'row_id', inv.row_id,
            'user_id', inv.user_id,
