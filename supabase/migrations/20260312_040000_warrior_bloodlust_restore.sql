-- =========================================================================================
-- MIGRATION: Restore Warrior Bloodlust to pvp_attack
-- =========================================================================================
-- Context: 20260307_090000_apply_monument_and_class_bonuses.sql added warrior_bloodlust_until
-- to public.users and implemented Warrior Bloodlust mechanics in pvp_attack.
-- 20260312_030000_fix_pvp_attack_null_safety.sql rewrote pvp_attack for NULL safety but
-- accidentally dropped the Warrior Bloodlust bonus (feature regression).
--
-- This migration restores:
--   1. Warrior Bloodlust active check: +10% attack damage when bloodlust is active
--   2. Warrior Bloodlust trigger: on win, set warrior_bloodlust_until = now() + 30 min
-- =========================================================================================
...
DECLARE
  ...
  v_daily_count INT;
  -- Warrior Bloodlust
  v_attacker_bloodlust BOOLEAN := false;
  v_defender_bloodlust BOOLEAN := false;
BEGIN
  ...
  -- Warrior Bloodlust: active if warrior class and bloodlust timestamp is set and not expired
  v_attacker_bloodlust := COALESCE(v_attacker.character_class, '') = 'warrior'
    AND v_attacker.warrior_bloodlust_until IS NOT NULL
    AND v_attacker.warrior_bloodlust_until > now();
  v_defender_bloodlust := COALESCE(v_defender.character_class, '') = 'warrior'
  ...
  FOR i IN 1..3 LOOP
    ...
    BEGIN
      ...
      v_atk_dmg := COALESCE(v_attacker.attack, 10) * (0.8 + random() * 0.4)
                   ...
                   * CASE WHEN v_attacker_bloodlust THEN 1.10 ELSE 1.0 END
      ...
      v_def_dmg := COALESCE(v_defender.attack, 10) * (0.8 + random() * 0.4)
                   ...
                   * CASE WHEN v_defender_bloodlust THEN 1.10 ELSE 1.0 END
  ...
  -- Update attacker: gold, rating, wins/losses, and Warrior Bloodlust on win
  UPDATE public.users
  SET gold       = gold + CASE WHEN v_winner_id = p_attacker_id THEN v_gold_stolen ELSE 0 END,
      ...
      -- Warrior Bloodlust: activated for 30 minutes on win
      warrior_bloodlust_until = CASE
        WHEN v_winner_id = p_attacker_id AND COALESCE(character_class, '') = 'warrior'
          THEN now() + interval '30 minutes'
        ELSE warrior_bloodlust_until
      END
  WHERE auth_id = p_attacker_id;

  -- Update defender: gold, rating, wins/losses, and Warrior Bloodlust on win
  UPDATE public.users
  SET gold       = gold - CASE WHEN v_winner_id = p_attacker_id THEN v_gold_stolen ELSE 0 END,
      ...
      -- Warrior Bloodlust: activated for 30 minutes on win
      warrior_bloodlust_until = CASE
        WHEN v_winner_id = p_defender_id AND COALESCE(character_class, '') = 'warrior'
          THEN now() + interval '30 minutes'
        ELSE warrior_bloodlust_until
      END
  ...
  v_hospital_chance := CASE WHEN v_winner_id = p_attacker_id THEN 0.05 ELSE 0.25 END;
  ...
  RETURN jsonb_build_object(
    ...
    'attacker_bloodlust',      v_attacker_bloodlust,
    'defender_bloodlust',      v_defender_bloodlust
  );
END;
