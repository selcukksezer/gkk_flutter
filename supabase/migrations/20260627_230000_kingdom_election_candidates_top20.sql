-- Krallık seçimi: aday listesi yalnızca aktif sezon sıralamasında ilk 20 lonca.

BEGIN;

CREATE OR REPLACE FUNCTION public._kingdom_election_candidate_guilds(p_limit INT DEFAULT 20)
RETURNS TABLE(guild_id uuid, guild_name text, season_rank integer)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  SELECT guild_id, guild_name, season_rank
  FROM ranked
  WHERE season_rank <= GREATEST(p_limit, 1)
...
BEGIN
  ...
  SELECT cg.season_rank
  INTO v_candidate_rank
  FROM public._kingdom_election_candidate_guilds(20) cg
  WHERE cg.guild_id = p_candidate_guild_id;

  IF v_candidate_rank IS NULL THEN
    RETURN json_build_object(
      'success', false,
      'error', 'Yalnızca sezon sıralamasında ilk 20 lonca aday olabilir.'
    );
  END IF;
...
BEGIN
  ...
  IF FOUND THEN
    ...
    FROM (
      SELECT
        cg.guild_id AS id,
        cg.guild_name AS name,
        cg.season_rank AS rank,
        COUNT(v.id)::integer AS vote_count
      FROM public._kingdom_election_candidate_guilds(20) cg
      LEFT JOIN public.kingdom_election_votes v
        ON v.candidate_guild_id = cg.guild_id AND v.election_id = v_election.id
      GROUP BY cg.guild_id, cg.guild_name, cg.season_rank
      ORDER BY cg.season_rank
    ) c;
...
GRANT EXECUTE ON FUNCTION public._kingdom_election_candidate_guilds(INT) TO authenticated;
